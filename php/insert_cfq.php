...
try {
    ...
    // Whitelist allowed columns for CFQ data
    $allowed_columns = ['prolificID', 'expName', 'timestamp', 'surveyOrder'];

    // Add question columns (q0 through q24)
    for ($i = 0; $i <= 24; $i++) {
        $allowed_columns[] = "q$i";
    }
    ...
    $stmt = $conn->prepare("INSERT INTO spaceprl_cfq ($columns) VALUES ($placeholders)");
    ...
    sendJsonResponse('success', 'CFQ data inserted successfully', ['inserted_id' => $conn->lastInsertId()]);

} catch (PDOException $e) {
    error_log("Database error in insert_cfq.php: " . $e->getMessage());
    sendJsonResponse('error', 'Database operation failed');
} catch (Exception $e) {
    error_log("General error in insert_cfq.php: " . $e->getMessage());
