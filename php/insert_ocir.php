...
try {
    ...
    // Whitelist allowed columns for OCI-R data
    $allowed_columns = ['prolificID', 'expName', 'timestamp', 'surveyOrder'];

    // Add question columns (q0 through q17)
    for ($i = 0; $i <= 17; $i++) {
        $allowed_columns[] = "q$i";
    }
    ...
    $stmt = $conn->prepare("INSERT INTO spaceprl_ocir ($columns) VALUES ($placeholders)");
    ...
    sendJsonResponse('success', 'OCI-R data inserted successfully', ['inserted_id' => $conn->lastInsertId()]);

} catch (PDOException $e) {
    error_log("Database error in insert_ocir.php: " . $e->getMessage());
    sendJsonResponse('error', 'Database operation failed');
} catch (Exception $e) {
    error_log("General error in insert_ocir.php: " . $e->getMessage());
