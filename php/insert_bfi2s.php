...
try {
    ...
    // Whitelist allowed columns for BFI-2-S data
    ...
    $stmt = $conn->prepare("INSERT INTO spaceprl_bfi2s ($columns) VALUES ($placeholders)");
    ...
    sendJsonResponse('success', 'BFI-2-S data inserted successfully', ['inserted_id' => $conn->lastInsertId()]);

} catch (PDOException $e) {
    error_log("Database error in insert_bfi2s.php: " . $e->getMessage());
    sendJsonResponse('error', 'Database operation failed');
} catch (Exception $e) {
    error_log("General error in insert_bfi2s.php: " . $e->getMessage());
